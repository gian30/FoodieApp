...
class AddPostViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    func uploadPost(){
        ...
        let uploadTask = postRef.putData(data as Data, metadata: nil) { (metadata, error) in
            ...
            self.ref = Database.database().reference()
            self.ref.child("users/\(self.user.uid)/posts").child("post\(self.randomString(len: 25))").setValue(["photo_url": downloadURL!, "desc": self.descOutlet.text as Any, "username":self.username, "uid":self.user.uid, "likes": 0, "isLast": true])
        }
